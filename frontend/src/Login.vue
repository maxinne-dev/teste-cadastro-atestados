    ...
    <Card>
      ...
      <form @submit.prevent="onSubmit">
        ...
        <FormField label="Senha" :error="errors.password" for="password">
          <BasePassword id="password" v-model="password" placeholder="••••••••" />
        </FormField>
        <label style="display:flex; gap:8px; align-items:center; margin-bottom: 16px;">
          <input type="checkbox" v-model="remember" /> Lembrar-me
        </label>
        <button class="btn primary" type="submit" :disabled="!canSubmit">Entrar</button>
      </form>
    </Card>
  </div>
...
<style scoped>
.btn { padding: 10px 12px; border-radius: var(--radius-md); border: 1px solid var(--color-border); background: var(--color-surface); cursor: pointer; }
