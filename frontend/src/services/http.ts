...
async function retryable<T>(fn: () => Promise<T>, attempts: number, baseDelay: number): Promise<T> {
...
export async function httpGet<T = unknown>(url: string, config?: Record<string, any>): Promise<T> {
  const exec = () => http.get<T>(url, config).then((r) => r.data);
  if (RETRY_ATTEMPTS > 0 && /^(GET)$/i.test((config?.method as string) || 'GET')) {
    return retryable(exec, RETRY_ATTEMPTS, RETRY_DELAY_MS);
  }
  return exec();
}

export async function httpPost<T = unknown>(url: string, data?: any, config?: Record<string, any>): Promise<T> {
  return http.post<T>(url, data, config).then((r) => r.data);
}
export async function httpPut<T = unknown>(url: string, data?: any, config?: Record<string, any>): Promise<T> {
  return http.put<T>(url, data, config).then((r) => r.data);
}
export async function httpPatch<T = unknown>(url: string, data?: any, config?: Record<string, any>): Promise<T> {
  return http.patch<T>(url, data, config).then((r) => r.data);
}
export async function httpDelete<T = unknown>(url: string, config?: Record<string, any>): Promise<T> {
