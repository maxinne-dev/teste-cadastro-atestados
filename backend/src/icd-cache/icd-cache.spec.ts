...
describe('IcdCacheService', () => {
  ...
  beforeEach(async () => {
    model = {
      findOneAndUpdate: jest.fn().mockReturnValue({ exec: jest.fn(async () => ({ code: 'J06.9', title: 'Old' })) }),
      findOne: jest.fn().mockReturnValue({ exec: jest.fn(async () => ({ code: 'J06.9', title: 'Acute...' })) }),
    }
  ...
  it('upsert uses findOneAndUpdate with upsert=true', async () => {
    await service.upsert('J06.9', 'Acute upper respiratory infection, unspecified', '2024-01')
    expect(model.findOneAndUpdate).toHaveBeenCalled()
    const [filter, update, options] = (model.findOneAndUpdate as jest.Mock).mock.calls[0]
