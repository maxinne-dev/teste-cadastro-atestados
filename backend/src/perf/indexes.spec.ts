...
describe('Index verification', () => {
  it('Collaborators: unique cpf and fullName text index', () => {
    ...
    const hasCpfUnique = idx.some(([fields, opts]: [any, any]) => fields.cpf === 1 && opts?.unique === true)
    const hasFullNameText = idx.some(([fields]: [any, any]) => fields.fullName === 'text')
  ...
  it('MedicalCertificates: compound collaboratorId+status, issueDate, and range startDate+endDate', () => {
    ...
    const hasCompound = idx.some(([fields]: [any, any]) => fields.collaboratorId === 1 && fields.status === 1)
    const hasIssue = idx.some(([fields]: [any, any]) => fields.issueDate === -1)
    const hasRange = idx.some(([fields]: [any, any]) => fields.startDate === 1 && fields.endDate === 1)
  ...
  it('Users: unique email and roles index', () => {
    ...
    const hasEmailUnique = idx.some(([fields, opts]: [any, any]) => fields.email === 1 && opts?.unique === true)
    const hasRolesIndex = idx.some(([fields]: [any, any]) => fields.roles === 1)
  ...
  it('ICD: unique code', () => {
    ...
    const hasUnique = idx.some(([fields, opts]: [any, any]) => fields.code === 1 && opts?.unique === true)
