...
export class JwtService {
  constructor(private readonly options: { secret: string; expiresIn?: string | number } = { secret: 'dev-secret' }) {}

  async signAsync(payload: any): Promise<string> {
    const header = { alg: 'HS256', typ: 'JWT' }
    const now = Math.floor(Date.now() / 1000)
    const expDelta = this.options.expiresIn ? parseTimespan(this.options.expiresIn) : 0
    const body = { ...payload, iat: now, ...(expDelta ? { exp: now + expDelta } : {}) }
