...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
      ...
      findByEmail: jest.fn(async (email: string) => {
        if (email === 'disabled@example.com')
          return {
            _id: '1',
            email,
            passwordHash: await new PasswordService().hash('secret123'),
            status: 'disabled',
            roles: [],
          } as any;
    ...
    const module = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: JwtService,
          useValue: new JwtService({ secret: 'test', expiresIn: '1h' }),
        },
      ],
    }).compile();
  ...
  it('rejects invalid credentials', async () => {
    await expect(
      auth.login('nope@example.com', 'secret123'),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  });

  it('rejects disabled users', async () => {
    await expect(
      auth.login('disabled@example.com', 'secret123'),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  });
