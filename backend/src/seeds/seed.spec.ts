jest.mock('mongoose', () => {
    ...
    const state: any = {
      ...
      find: jest.fn((filter: any) => {
        ...
        if (name === 'collaborators') {
          const docs = (mockData.mockCollaborators || []).map((c: any) => ({ ...c, _id: `id:${c.cpf}` }))
  ...
  const connection = { readyState: 1, collection: (name: string) => collections[name] || makeCollection(name) }
  const mocked: any = { connection, connect: jest.fn(async () => mocked), __calls: calls }
  return { __esModule: true, default: mocked, ...mocked }
})

import * as mongoose from 'mongoose'
import { mockCollaborators, mockUsers, mockIcdCodes, mockCertificates } from './mock-data'
...
describe('Seed script scaffold', () => {
  it('runs idempotently, writes meta, seeds core docs and links certificate to collaborator', async () => {
    ...
    const collabOps = calls.updateOne.filter((c: any) => c[0] === 'collaborators')
    expect(collabOps.length).toBeGreaterThanOrEqual((mockCollaborators || []).length)
    ...
    const certOps = calls.updateOne.filter((c: any) => c[0] === 'medicalcertificates')
    expect(certOps.length).toBeGreaterThanOrEqual((mockCertificates || []).length)
