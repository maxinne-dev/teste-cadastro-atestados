...
import { MedicalCertificate, MedicalCertificateDocument } from './medical-certificate.schema.js'
...
export class MedicalCertificatesService {
  ...
  async create(input: Partial<MedicalCertificate> & { collaboratorId: string }) {
    const doc: any = { ...input }
  ...
  async cancel(id: string) {
    ...
    await this.model.updateOne({ _id }, { $set: { status: 'cancelled' } }).exec()
