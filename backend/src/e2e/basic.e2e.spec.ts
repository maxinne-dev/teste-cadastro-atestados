...
describe('E2E: Health and Auth flow', () => {
  ...
  it('auth + protected route: 401, 403, then 200', async () => {
    // 401 without token
    await request(app.getHttpServer()).get('/api/users/admin@example.com').expect(401)

    // 403 with HR token (no admin role)
    const jtiHr = 'jti-hr-1'
    const hrToken = await jwt.signAsync({ sub: 'u-hr', email: 'hr@example.com', roles: ['hr'], jti: jtiHr })
