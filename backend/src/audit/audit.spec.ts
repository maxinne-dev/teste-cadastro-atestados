...
describe('AuditService', () => {
  ...
  beforeEach(async () => {
    ...
    const module = await Test.createTestingModule({
      providers: [AuditService, { provide: getModelToken('AuditLog'), useValue: model }],
    }).compile()
  ...
  it('record persists an event', async () => {
    await service.record({ action: 'sensitive_data_access', actorUserId: 'u1', resource: '/api/x', ip: '127.0.0.1' })
...
describe('AuditLogSchema indexes', () => {
  it('has timestamp index and optional TTL when ttlDays provided', () => {
    ...
    const hasTimestampIdx = idxNoTtl.some(
      ([fields]: [Record<string, any>, Record<string, any>]) => fields.timestamp === -1
    )
    ...
    const hasTtl = idxWithTtl.some(
      ([fields, opts]: [Record<string, any>, Record<string, any>]) =>
        fields.timestamp === 1 && typeof opts?.expireAfterSeconds === 'number' && opts.expireAfterSeconds > 0
    )
